...
description: One-command connect of the builder's Claude Desktop to their Example dev box (ADR 0031, #685). Triggers when the user says "/builder-connect", "connect me to the dev box", "set up my dev box", "connect Claude Desktop to the box", or "onboard me to the box". Runs the self-contained bootstrap that signs the builder in with GitHub, creates + registers their SSH key, and installs the Claude Desktop connection file — then tells them to restart Desktop and pick "Example Dev Box". This is the BUILDER side; the OPERATOR side is `bongos exec scripts/gds/box.js onboard <login>`.
---
...
4. **Confirm the finish state and give the next steps:**
   ...
   - Open **Code → Environment → "Example Dev Box"** to connect.
