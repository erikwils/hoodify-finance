...
You are filing a vulnerability report on the Example project. The reporter is a trusted builder who has found a flaw — in the GDS, the game server, the chat broadcast, the deploy chain, the auth model — and wants the red-team bounty surface to pick it up.
...
- if confirmed, the city pays out example by severity.
...
2. **Target.** One short line — the route, file, endpoint, or surface. Examples: `POST /api/gds/tasks/:id/override-request`, `src/bongos/auth.js#verifyToken`, `caddy: example.com:443`. Goes into the `target` column verbatim.
...
Bounty if confirmed: <example> (see /public/bounty-table).
