...
- **Promote → task** (the common case). Ask for version, title, est_minutes, priority, credits_reward (`touches[]` optional — [ADR 0049](../../../docs/adr/<redacted>.md)). (Filing a task *under a goal* — stamping `tasks.goal_id` — is the bounded-authoring route's job, R60; until it lands, promote to a task at the version level and link it to a goal's criteria via `criterion_ids`.)
