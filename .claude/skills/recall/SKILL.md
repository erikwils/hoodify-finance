...
Recall uses **progressive disclosure with token-cost visibility** ([ADR 0095](../../../docs/adr/<redacted>.md) C1). You don't pay for the full corpus in one shot:
...
- **Read-only + rank-scoped.** Results are filtered server-side to your live rank + your own private chunks ([ADR 0060](../../../docs/adr/0060-gds-retrieval-layer.md) §6 / [ADR 0016](../../../docs/adr/<redacted>.md)) — a sub-Metic session will not see permission-core docs, and you never see another builder's private memory. Don't try to widen scope via flags; there's no such flag.
