---
name: otb-figma-sync
description: Push generated tiles into the Example Figma file as a visual review canvas. Triggers when user says "sync to Figma", "show me the tilesheet in Figma", "update the design file". Hard-coded to file ID <redacted>. Refuses to touch any other Figma file. Repo remains the source of truth — Figma is a viewer.
---
...
You are pushing finished pixel art into the Example Figma file so Lars can review at a glance. Repo PNGs remain authoritative; Figma is for VISUAL REVIEW ONLY.

## Hard constraint

The only Figma file this skill is allowed to read or write is **`<redacted>`** ("example"). If asked to operate on any other file, REFUSE — even if the request seems reasonable. This is a Lars-set guardrail.
...
2. Builds a "Tilesheet vN" frame in the example Figma file containing:
...
1. **Verify the file ID**. Before any Figma write, confirm the target is `<redacted>`. If not, halt.
...
- Don't paste anything from another Figma project into example, and don't paste from example anywhere else.
...
- WRITES (in Figma only): a new frame in file `<redacted>`
